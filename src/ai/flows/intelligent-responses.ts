...
const intelligentResponseFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      console.error(`Model ${modelName} failed. Falling back to default.`, error);
      try {
        const fallbackModel = googleAI.model('googleai/gemini-1.5-flash-latest');
        const promptInput = { context: input.context, question: input.question };
        const { output } = await intelligentResponsePrompt(promptInput, { model: fallbackModel });
        
        if (!output) {
          throw new Error('Fallback AI model also returned an empty response.');
        }
        // Add a note that the response is from a fallback model
        const answerWithNote = `${output.answer}\n\n*(Note: The selected model failed. This response was generated by a fallback model.)*`;
        return { answer: answerWithNote };

      } catch (fallbackError: any) {
          console.error(`Fallback model failed.`, fallbackError);
          throw new Error(
            `The selected AI model (${modelName}) and the fallback model both failed to respond. Details: ${fallbackError.message}`
          );
      }
    }
  }
